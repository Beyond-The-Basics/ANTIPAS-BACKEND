...
from app.models.enums import DisputeStatus, FeedbackContext, PartyType
...
class Feedback(UUIDPKMixin, TimestampMixin, Base):
    ...
    context: Mapped[FeedbackContext] = mapped_column(Enum(FeedbackContext, native_enum=False, length=16))
    from_type: Mapped[PartyType] = mapped_column(Enum(PartyType, native_enum=False, length=8))
    from_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True))
    to_type: Mapped[PartyType] = mapped_column(Enum(PartyType, native_enum=False, length=8))
...
class Dispute(UUIDPKMixin, TimestampMixin, Base):
    ...
    status: Mapped[DisputeStatus] = mapped_column(
        Enum(DisputeStatus, native_enum=False, length=24), default=DisputeStatus.OPEN
    )
